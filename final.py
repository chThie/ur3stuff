...
class LeapThread(Thread):
    def __init__(self, output_text, result_text, pumps):
        Thread.__init__(self)
        self.listener = HandGestureListener(output_text, result_text, pumps)
...
class QAppThread(Thread):
    def __init__(self):
        Thread.__init__(self)
        self.app = QApplication(sys.argv)
        self.window = HandGestureWindow()
        self.window.show()
...
def main():
    ...
    qapp_thread = QAppThread()
    qapp_thread.start()
    qapp_thread.app.exec_()

    leap_thread = LeapThread(qapp_thread.window.update_output_text, qapp_thread.window.update_result_text,
                             qapp_thread.window.pumps)
    leap_thread.start()
    time.sleep(0.5)
